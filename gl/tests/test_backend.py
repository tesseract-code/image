...
def reset_gl_backend():
    for module in list(sys.modules.keys()):
        if module.startswith("gl_backend") or module in ("OpenGL", "OpenGL.GL", "OpenGL.GLU"):
...
def mock_cross_platform_modules(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "cross_platform.qt6_utils.image.gl.config", fake_config_module)
    monkeypatch.setitem(sys.modules, "cross_platform.qt6_utils.image.gl.debug", fake_debug_module)
...
def mock_opengl(monkeypatch):
    ...
    gl_mock.glGetStringi.side_effect = lambda _, i: [b"GL_ARB_texture_storage", b"GL_EXT_other"][i]
...
def test_import_without_pyopengl(mock_cross_platform_modules, monkeypatch):
    ...
    with patch("builtins.__import__", side_effect=failing_import):
        with pytest.raises(ImportError):
            import cross_platform.qt6_utils.image.gl.backend as gl_backend


def test_debug_mode_enabled_pyopengl_flags(mock_cross_platform_modules, mock_opengl, monkeypatch):
...
def test_debug_mode_disabled(mock_cross_platform_modules, mock_opengl, monkeypatch):
...
def test_macos_disables_immutable_storage_and_logs_info(mock_cross_platform_modules, mock_opengl, monkeypatch, caplog):
    monkeypatch.setenv("GL_DEBUG_MODE", "1")
...
def test_macos_no_debug_callback_and_no_warning(mock_cross_platform_modules, mock_opengl, monkeypatch, caplog):
    monkeypatch.setenv("GL_DEBUG_MODE", "1")
    monkeypatch.setattr("platform.system", lambda: "Darwin")
...
def test_gl_version_4_2_or_higher_immutable(mock_cross_platform_modules, mock_opengl, monkeypatch):
    monkeypatch.setattr("platform.system", lambda: "Linux")
...
def test_gl_version_4_1_with_arb_extension(mock_cross_platform_modules, mock_opengl, monkeypatch):
    monkeypatch.setattr("platform.system", lambda: "Linux")
    _, gl_mock, _ = mock_opengl
    gl_mock.glGetString.return_value = b"4.1.0"
    gl_mock.glGetIntegerv.return_value = 2
    gl_mock.glGetStringi.side_effect = [b"GL_ARB_texture_storage", b"GL_EXT_other"]
...
def test_gl_version_4_1_without_extension(mock_cross_platform_modules, mock_opengl, monkeypatch):
    monkeypatch.setattr("platform.system", lambda: "Linux")
    _, gl_mock, _ = mock_opengl
    gl_mock.glGetString.return_value = b"4.1.0"
...
def test_gl_version_below_4_fallback_extension_string(mock_cross_platform_modules, mock_opengl, monkeypatch):
    monkeypatch.setattr("platform.system", lambda: "Linux")
    _, gl_mock, _ = mock_opengl
...
def test_gl_version_string_parsing(mock_cross_platform_modules, mock_opengl, monkeypatch):
    monkeypatch.setattr("platform.system", lambda: "Linux")
    _, gl_mock, _ = mock_opengl
...
def test_context_failure_fallback(mock_cross_platform_modules, mock_opengl, monkeypatch, caplog):
    monkeypatch.setattr("platform.system", lambda: "Linux")
    _, gl_mock, _ = mock_opengl
...
def test_initialize_context_idempotent(mock_cross_platform_modules, mock_opengl, monkeypatch):
    monkeypatch.setattr("platform.system", lambda: "Linux")
    gl_backend = _import_gl_backend()
    gl_backend.initialize_context()
...
def test_warning_if_opengl_already_imported(mock_cross_platform_modules, mock_opengl, monkeypatch, caplog):
...
def test_gl_and_glu_are_exported(mock_cross_platform_modules, mock_opengl, monkeypatch):
    monkeypatch.setattr("platform.system", lambda: "Linux")
    gl_backend = _import_gl_backend()
